...
variable "ScannerLambdaAliasARN" {
  type = string
  default = ""
  description = ""
}
...
variable "ScanOnGetObject" {
  type = bool
  default = false
  description = "Scan objects retrieved (GET requests) from S3 with the Object Lambda Access Point. Enabling this option requires that the storage stack is deployed in the same account and same region as the scanner stack. For more information, see https://cloudone.trendmicro.com/docs/file-storage-security/aws-scan-on-get-object/"
}
